  ...
  return (
    <div className="flex flex-col gap-7">
      {/* Mascot stays mounted across questions (Rive is heavy to re-init); only
          the bubble + answer block re-animate per question via AnimatedGroup. */}
      <div className="flex items-center gap-3.5 md:gap-6">
        <QuizMascot correctKey={correctKey} />
        <AnimatedGroup key={currentId} preset="fade" className="min-w-0 flex-1">
          <QuizBubble stem={question.stem} />
        </AnimatedGroup>
      </div>

      <AnimatedGroup key={currentId} preset="fade" className="flex flex-col gap-7">
